...
export function buildToolInstructions(specs: ToolSpec[]): string {
	...
	return [
		...
		...workflowNote,
		'',
		`Rules: when you need tool information, reply with ONLY vscode-tool blocks (up to ${MAX_REQUESTS_PER_TURN} per reply) and no other prose; the editor runs them and sends the results back to you. After receiving results you may request more tools or give your final answer. Tackle multi-step work one step per reply: for a multi-step request, give the plan in a tool-free reply first, then take one step (one short lead-in sentence plus its block) per following reply; a single lookup is one step, so answer it tool-first. Never guess at file contents, workspace structure, or live Rewst data when a tool can check it. Long Rewst Buddy tool results include a preview and a saved file path; use read_file or VS Code search tools on that path instead of repeating the same request.${graphqlFirstRule}`,
	].join('\n');
}
