...
const WORKFLOW_BULLET = `**Rewst workflows → the purpose-built workflow tools, first.** When the task touches a specific Rewst WORKFLOW — reading its structure, editing it, laying it out, running it, or debugging an execution — reach for the dedicated editor tools before raw GraphQL or any native wrapper: \`buddy_workflow_search\` (find a workflow by name across every org you can access — resolve its id with this instead of guessing or listing via GraphQL), \`buddy_workflow_get\` (read a workflow as a node/edge graph), \`buddy_action_search\` (find or describe actions), \`buddy_workflow_edit\` (apply high-level operations — add/connect tasks, set inputs, etc.), \`buddy_workflow_autolayout\`, \`buddy_workflow_run\` (trigger a run and wait for the outcome), \`buddy_workflow_executions\` (list recent runs), \`buddy_execution_logs\` (per-task logs of one run — the fast path to WHY it failed), and \`buddy_render_jinja\` (evaluate a Jinja expression against a real execution before you change it). They bundle the GraphQL choreography, encode the platform's quirks (full-graph replace, version tokens, transition ordering, safe task defaults), and return far less noise than raw queries. To LIST or FIND workflows — by name, or all of them in an org — use \`buddy_workflow_search\` (omit the query to list, or pass orgId to scope), not a raw GraphQL query and never the native \`listWorkflow\` / \`searchWorkflows\`. Drop to \`buddy_graphql_read\` only for workflow data these do not expose. Use \`buddy_graphql_mutate\` only as an unsafe last resort for workflow changes that the workflow tools cannot express, and a native platform wrapper only after both editor-tool paths have failed.`;

const GRAPHQL_BULLET = `**Other live Rewst data → GraphQL reads, before native wrappers.** For integrations, org variables, triggers, scripts, templates, forms, or any other platform entity NOT handled by the workflow tools above, your read action should be \`buddy_graphql_schema\` or \`buddy_graphql_read\` — discover types and fields, then query. Running a native platform tool first is an error, even when one with a matching name exists. Your built-in platform tools — \`listWorkflow\`, \`searchWorkflows\`, \`readIntegration\`, \`searchActionsByNameOrDescription\`, \`listOrgVariables\`, and every similar wrapper — are the LAST resort: they paginate poorly, drop fields, and cannot express filters that GraphQL can. Do NOT call them before safe GraphQL reads have been tried, even though they run without an editor round-trip and feel faster. "What org variables are set?" means schema introspection plus a GraphQL read query, not \`listOrgVariables\` ("list the workflows", by contrast, is \`buddy_workflow_search\`, not GraphQL). Fall back to a native tool only after a GraphQL read attempt has actually failed or for a capability GraphQL does not expose (ranked search, option population) — and say that you fell back. Never declare data unavailable until both paths have been tried. For changes, prefer purpose-built workflow/editor tools. Use \`buddy_graphql_mutate\` only when it appears in the available tool list and no safer editor tool can express the change; it is unsafe and the last resort. Your first action for live data not covered above is a \`buddy_graphql_schema\` or \`buddy_graphql_read\` vscode-tool block, emitted directly.`;
...
export function buildEngineeringDirective(availableTools: ReadonlySet<string>): string {
	...
	const hasGraphql =
		availableTools.has('buddy_graphql_read') ||
		availableTools.has('buddy_graphql_schema') ||
		availableTools.has('buddy_graphql_mutate');
