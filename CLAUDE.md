...
- **Write tools (`access:'write'`) are org-scoped and gated twice.** A write capability is exposed only when `rewst-buddy.mcp.enableWriteTools` (or `enableDangerousGraphqlMutation` for `rewst_graphql_mutate`) is on, and may only target an org on `rewst-buddy.mcp.writeOrgAllowlist`. The allowlist is enforced centrally in `McpActions.callTool` (`assertOrgWriteAllowed`) for every `access:'write'` capability before it runs: empty means "any managed org" (logged as a warning), otherwise a write to an unlisted org is rejected with `org_not_allowlisted`. It is a setting, never a tool, so the model can't widen it; reads are never restricted. No write tool may be `requiresOrg:false` (the allowlist needs a concrete orgId), and any by-id write must re-verify the resource's `orgId` against the requested org before mutating (one session can manage several orgs). The per-call approval prompt is hosted in VS Code and may not surface to an external MCP client, so the allowlist — not the prompt — is the reliable blast-radius gate; keep that in mind when wording the `approval_required` message.
