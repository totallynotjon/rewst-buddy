...
- **Trigger field-name traps (updateTrigger / the `Trigger` type).** These caused an unintended production activation change (#181); the shared helper in `triggerUpdate.ts` encodes them, so route trigger edits through it rather than hand-writing `updateTrigger`.
