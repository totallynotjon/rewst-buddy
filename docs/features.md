...
- **Context usage** — Rewst reports how much of the conversation's context window it's using, surfaced as an inline line (`📊 Context: 42% · 60.5K / 144K tokens`). VS Code's own "Context Window" gauge stays at `0` for Cage-Free Rewsty because a third-party model provider can't feed it usage ([microsoft/vscode#309207](https://github.com/microsoft/vscode/issues/309207), [#313458](https://github.com/microsoft/vscode/issues/313458)), so this is the live read instead. It follows the same `rewst-buddy.ai.showActivity` toggle as the activity lines
