...
- **Workflows** (`workflows` in `rewst-buddy.ai.tools`) — purpose-built tools that let Cage-Free Rewsty understand and edit Rewst workflows without the trial-and-error of raw GraphQL:
    ...
    - Reads run directly; **edits, auto-layout, and runs each require an inline chat confirmation** showing what will change or run — the same approval surface as a GraphQL mutation. **Every one is confirmed individually each time**: the extension doesn't remember approvals and ships these tools as **not eligible for auto-approval**, so the prompt can't be allow-listed away (short of VS Code's own Autopilot / Bypass-Approvals mode). Off by default because the session can change any workflow you can in Rewst
