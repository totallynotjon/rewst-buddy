...
When any Rewst Buddy AI tool returns a large result, the chat shows a preview and saves the full output to an ignored `.rewst-buddy/tool-results/` file in the workspace. Cage-Free Rewsty can inspect that path with `read_file` or search through it with VS Code's search tools instead of rerunning the Rewst request.
