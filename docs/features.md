...
- **GraphQL** (`rewst-buddy.ai.enableGraphqlTool`) — `rewst_graphql` composes and runs GraphQL operations against your Rewst instance using your session, with `rewst_graphql_schema` for exploring the schema. Queries run directly; **mutations always require an inline chat confirmation (Continue / Cancel) showing the full operation** — the same approval surface as Cage-Free Rewsty's other Rewst-side actions, not a separate OS dialog. Every mutation must declare a **scopeId** — a stable id of the single resource it changes (e.g. the workflow id) — and approval is remembered per scopeId for the session: confirm one change to a workflow and further edits to **that** workflow run without re-asking, while a different workflow (or a mutation that names no resource) is confirmed again. Off by default because the session can read and change anything you can in Rewst
