...
- `buddy_graphql_schema` and `buddy_graphql_read` — inspect and query your Rewst GraphQL API using your active session. These read-only tools are available without a settings gate so Cage-Free Rewsty can look up live Rewst data instead of guessing. They reject mutations and subscriptions
...
Off by default because they let a remote assistant direct activity on your machine or mutate your Rewst instance. Enable them by checking the corresponding box in the single **`rewst-buddy.ai.tools`** setting (`web`, `graphqlUnsafe`, `workflows`; `workspace` is on by default):
