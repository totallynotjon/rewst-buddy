...
Off by default because they let a remote assistant direct activity on your machine. Enable them by checking the corresponding box in the single **`rewst-buddy.ai.tools`** setting (`graphql`, `workflows`; `workspace` is on by default):
