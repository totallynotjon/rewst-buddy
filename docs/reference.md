...
| `rewst-buddy.mcp.enabledTools`      | `array`   | `[]`                                                                                                                                 | Allowlist of MCP capability names to expose. Leave empty to expose all enabled read tools (the default); set names to restrict the surface (e.g. `["list_orgs", "list_templates", "list_template_links"]`). A malformed (non-array) value disables the MCP server until corrected.                        |
...
| `rewst-buddy.ai.tools`              | `array`   | `["workspace"]`                                                                                                                      | Rewst MCP capability families to expose. `workspace` enables linked-template metadata (`list_template_links`); `graphql` enables schema/query/mutation MCP tools; `workflows` enables workflow helper MCP tools. Write tools also require `rewst-buddy.mcp.enableWriteTools` and approval inside VS Code. |
