...
| `rewst-buddy.ai.tools`                  | `array`   | `["workspace"]`                                                                                                                      | Extra AI tools Cage-Free Rewsty may use — check the ones to enable. `workspace` shares a workspace overview and the template-link list; `web` lets the assistant search the public web; `workflows` lets it read and edit Rewst workflows; `graphqlUnsafe` allows last-resort raw GraphQL mutations. Safe GraphQL schema/read tools are available without a settings gate. Rewst-side edits always require your approval. |
