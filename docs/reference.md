...
| `rewst-buddy.mcp.enabledTools`                   | `array`   | `[]`                                                                                                                                 | Allowlist of MCP capability names to expose. Leave empty to expose all capabilities allowed by the three MCP switches; set names to restrict the surface (e.g. `["list_orgs", "list_templates", "list_template_links"]`). A malformed (non-array) value disables the MCP server until corrected. |
