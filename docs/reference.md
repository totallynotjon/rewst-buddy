...
| `rewst-buddy.ai.enableGraphqlTool`    | `boolean` | `false`                                                     | Let Cage-Free Rewsty compose and run GraphQL operations against your Rewst instance using your session. Queries run directly; mutations always require your approval through VS Code's inline chat confirmation (Continue / Cancel) showing the full operation, must declare the resource and org they change (scopeId, scopeName, orgId, orgName), and approval is remembered for the session by the ids. Off by default because the session can read and change anything you can in Rewst. |
| `rewst-buddy.ai.enableWorkflowTools`  | `boolean` | `false`                                                     | Let Cage-Free Rewsty read and edit Rewst workflows through high-level tools (read a workflow as a node/edge graph, search actions, apply add/connect/update/reposition operations) using your session. Reads run directly; edits always require your approval through VS Code's inline chat confirmation showing the operations, and approval is remembered per workflow for the session. Off by default because the session can change any workflow you can in Rewst.                       |
