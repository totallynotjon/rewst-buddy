...
| `rewst-buddy.ai.enableGraphqlTool`    | `boolean` | `false`                                                     | Let Cage-Free Rewsty compose and run GraphQL operations against your Rewst instance using your session. Queries run directly; mutations always require your approval through VS Code's inline chat confirmation (Continue / Cancel) showing the full operation, must declare a scopeId for the resource they change, and approval is remembered per scopeId for the session. |
