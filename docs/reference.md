...
| `rewst-buddy.ai.enableGraphqlTool`    | `boolean` | `false`                                                     | Let Cage-Free Rewsty compose and run GraphQL operations against your Rewst instance using your session. Queries run directly; mutations always require your approval through VS Code's inline chat confirmation (Continue / Cancel) showing the full operation, must declare scopeId, scopeName, orgId, and orgName for the resource and org they change, and approval is remembered for the session by the ids. |
