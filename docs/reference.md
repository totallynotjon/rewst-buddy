...
| `rewst-buddy.ai.anthropicProxy`                  | `boolean` | `false`                                                                                                                              | Serve an Anthropic Messages API endpoint (`POST /v1/messages`) on the local Rewst Buddy server, backed by the Rewst AI assistant. Local tools that speak the Anthropic API (e.g. Claude Code with `ANTHROPIC_BASE_URL`) can then use Cage-Free Rewsty as their model. Requests authenticate with the same local token as the MCP server. See [Anthropic Messages Proxy](features.md#anthropic-messages-proxy) for setup instructions.                                                                 |
