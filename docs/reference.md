...
| `rewst-buddy.mcp.enableWriteTools`               | `boolean` | `false`                                                                                                                              | Allow workflow MCP tools that change Rewst data (`buddy_workflow_edit`, `buddy_workflow_autolayout`, and `buddy_workflow_run`). Even when enabled, each change still requires your approval inside VS Code. Does not expose raw GraphQL mutation.                                                                                                                                                                               |
| `rewst-buddy.mcp.enableDangerousGraphqlMutation` | `boolean` | `false`                                                                                                                              | Allow the raw `rewst_graphql_mutate` MCP tool to run arbitrary GraphQL mutations against the live Rewst organization. This is separate from write tools because it can change anything your Rewst session can change; each mutation still requires per-resource approval inside VS Code.                                                                                                                                        |
| `rewst-buddy.mcp.alwaysAllowedOrgs`              | `array`   | `[]`                                                                                                                                 | Organization ids that are always in the [working scope](features.md#working-scope), regardless of what you pin. Tools may always operate on these orgs (e.g. a sandbox). The working scope and this list form the effective allowed set: a write must target an org in it, so with nothing pinned and this empty, writes are blocked. Find org ids with `list_orgs`. (Replaces `writeOrgAllowlist`; old values are still read.) |
