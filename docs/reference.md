...
| ------------------------------------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rewst-buddy.regions`                            | `array`   | `[{"name":"North America","cookieName":"appSession","graphqlUrl":"https://api.rewst.io/graphql","loginUrl":"https://app.rewst.io"},{"name":"United Kingdom","cookieName":"euAppSession","graphqlUrl":"https://api.eu.rewst.io/graphql","loginUrl":"https://app.eu.rewst.io"},{"name":"Asia","cookieName":"auAppSession","graphqlUrl":"https://api.rewst.asia/graphql","loginUrl":"https://app.rewst.asia"},{"name":"Europe","cookieName":"deAppSession","graphqlUrl":"https://api.rewst.eu/graphql","loginUrl":"https://app.rewst.eu"}]` | Regional configuration for different Rewst instances. See [Multi-Region Setup](#multi-region-setup).                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `rewst-buddy.mcp.enable`                         | `boolean` | `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Expose your authenticated Rewst sessions to external MCP clients (Claude Desktop, Claude Code, Cursor) through a local, credential-free bridge. Off by default: enabling it exposes read capabilities through your session. Review the security notes before enabling.                                                                                                                                                                                                                                |
| `rewst-buddy.mcp.enableWriteTools`               | `boolean` | `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Allow MCP tools that change Rewst data: workflow editing, auto-layout, and runs; workflow create/delete; template create, edit, rename, delete, and sync; and org-variable, tag, and trigger changes. Even when enabled, each change still requires your approval inside VS Code and may only target an org in your working scope or always-allowed list. Workflow edits save tasks with sequential graph defaults and do not expose task parallelism controls. Does not expose raw GraphQL mutation. |
| `rewst-buddy.mcp.enableDangerousGraphqlMutation` | `boolean` | `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Allow the raw `buddy_graphql_mutate` MCP tool to run arbitrary GraphQL mutations against the live Rewst organization. This is separate from write tools because it can change anything your Rewst session can change; each mutation still requires per-resource approval inside VS Code.                                                                                                                                                                                                              |
| `rewst-buddy.mcp.alwaysAllowedOrgs`              | `array`   | `[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Organization ids that are always in the [working scope](features.md#working-scope), regardless of what you pin. Tools may always operate on these orgs (e.g. a sandbox). The working scope and this list form the effective allowed set: a write must target an org in it, so with nothing pinned and this empty, writes are blocked. Find org ids with `buddy_list_orgs`. (Replaces `writeOrgAllowlist`; old values are still read.)                                                                 |
| `rewst-buddy.mcp.workingOrgScope`                | `string`  | `strict`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | How the [working scope](features.md#working-scope) constrains reads: `strict` scopes reads to the working orgs once one is pinned; `writes` leaves reads cross-org and only gates writes.                                                                                                                                                                                                                                                                                                             |
| `rewst-buddy.mcp.exportRoots`                    | `array`   | `[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Additional absolute directories approved for `buddy_export_workflows` local output. Downloads, active workspace folders, and the current Git checkout are approved automatically. Export paths stay within the approved directories.                                                                                                                                                                                                                                                                  |
| `rewst-buddy.mcp.exportDefaultDir`               | `string`  | `""`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Default directory `buddy_export_workflows` saves to when no `outputPath` is given. Must be an absolute path; it is created when missing and automatically approved. Empty uses the cross-OS default (`<home>/Downloads/Rewst Exports`).                                                                                                                                                                                                                                                               |
...
| `rewst-buddy.ai.maxBuddyToolRounds`              | `number`  | `8`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Maximum in-process Buddy tool calls Cage-Free Rewsty may run within one chat response before stopping and asking you to continue (range 1–100).                                                                                                                                                                                                                                                                                                                                                       |

## Multi-Region Setup

North America (US), United Kingdom (UK), Asia (AU), and Europe (DE) are built
in. For other Rewst instances, configure custom regions in VS Code settings:
...
Each region entry requires `name`, `cookieName`, `graphqlUrl`, and `loginUrl`. An optional `subscriptionsUrl` sets the WebSocket endpoint used by the AI assistant; when omitted it is derived from `graphqlUrl` (e.g. `https://api.rewst.io/graphql` → `wss://api.rewst.io/subscriptions`). Jinja filter documentation uses the matching regional engine host, such as `https://engine.eu.rewst.io` for UK and `https://engine.rewst.eu` for DE; subscriptions use the matching `/subscriptions` endpoint.
