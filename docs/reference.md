...
| `rewst-buddy.ai.tools`              | `array`   | `["workspace"]`                                                                                                                      | Extra AI tools Cage-Free Rewsty may use, shown as checkboxes: `workspace` (share a workspace overview + template-link list), `web` (public web search; private/loopback hosts blocked), `graphql` (run GraphQL against your Rewst instance), `workflows` (read/edit workflows, search actions, render Jinja against executions). GraphQL and workflow **edits always require your approval** in the chat. Only `workspace` is on by default — the others let a remote assistant act against the web or your Rewst instance. |
