...
- **Template sync over MCP** — Two tools let an AI assistant drive the link-and-sync workflow: `buddy_template_sync_status` reports a linked file's state (in-sync/conflict, read-only) and `buddy_template_sync` pushes or pulls it, stopping on a conflict instead of overwriting. Sync is a gated write tool. (#21)
- **Large AI tool results** — Cage-Free Rewsty now caches oversized tool output in memory and returns a preview plus a result id, so it can page or search the full result with `buddy_result_read` instead of rerunning. Cache size: `rewst-buddy.ai.toolResultCacheLimitMB` (default 500 MB). (#48)
- **MCP server (experimental): workflow tools** — external MCP clients can now read, debug, and edit Rewst workflows through the same helpers the Cage-Free Rewsty chat uses. Read tools come with workflow tools enabled; the three write tools each require a per-edit approval inside VS Code. (#68)
**Working scope: pin which orgs (and workflows) Rewst tools may touch.** Set it from the status bar or _Set Working Scope_; writes — in chat and over MCP — stay within it and are blocked until you pin an org or list one in `alwaysAllowedOrgs`, with reads scoped too under strict mode. (#87)
- **`buddy_template_link_status` tool** — Check whether one specific file is linked to a Rewst template, returning its template, org, and sync-on-save state from local state without a network call. (#92)
- **Jinja filter docs tool** — a new `buddy_get_jinja_filter_docs` MCP tool serves the full documentation for Rewst's built-in Jinja filters, so the assistant can look up what a filter does and its signature by name or keyword while writing templates. (#93)
...
- **Sub-workflow visibility in execution logs** — `buddy_execution_logs` now marks tasks that spawned sub-workflow executions and, with `includeSubExecutions`, inlines the child runs' task logs. (#126)
- **MCP instructions + recipe prompts** — the MCP server now sends working-method guidance to clients on connect, and exposes `debug-execution`, `safe-workflow-edit`, and `compose-sub-workflow` prompts. (#136)
...
**Impact and Crate discovery tools** — `buddy_workflow_impact` lists the workflows that would break before a sub-workflow's inputs/output change or a pack action upgrade, and `buddy_search_crates` finds prebuilt Rewst Crates (with per-org install status) so agents reuse instead of rebuilding.
...
- **Jinja IntelliSense for linked files.** Get autocomplete and hover docs for Rewst's built-in Jinja filters, template-name completion inside `template("...")` calls, and basic keyword highlighting for Rewst's Jinja dialect.
- **Live Jinja preview** — preview how the active linked template renders against a real workflow
  execution's context, updating as you type, via "Preview Jinja Render" (`Ctrl+Shift+Enter` /
  `Cmd+Shift+Enter`). Opens as native editor panes with a "Pick Jinja Preview Context" command for
  layering manual variable overrides on top of the execution's context.
- Added the buddy_find_executions_by_variable MCP tool, which finds executions of a workflow whose input, output, or context variable matches a name (and optional value).
- Add the `buddy_list_org_variables` read-only MCP tool for masked organization configuration variables.
- Add the `buddy_find_action` read-only MCP tool for searching installed pack actions.
- **MCP server (experimental): audit logging** — every external MCP tool call is now logged to the Rewst Buddy output channel as one `[MCP audit]` line (tool, org, outcome, duration). Argument values, GraphQL query bodies, and secrets are never logged. (#64)
...
- Oversized Buddy tool results now return a preview plus cached id, so agents can page or search the full result with `buddy_result_read`. (#70)
- **Deep-clone a template bundle over MCP** — `buddy_template_bundle_clone` deep-copies a template and everything it references (transitively) into a target org behind one approval, rewriting references to the new ids and rolling back on failure. Cross-org or deleted references are reported, not dropped. (#85)
- **Template link management over MCP** — New tools `buddy_template_link`, `buddy_template_unlink`, and `buddy_template_sync_on_save` let an assistant manage local file ↔ template links (local-only, no Rewst writes), pairing with the sync tools to drive the full link-edit-sync workflow without the VS Code UI. (#85)
- **MCP server (experimental): Rotate MCP Token command** — a new **Rotate MCP Token** command revokes MCP access by minting a fresh endpoint token, immediately cutting off any client still using the old one. It confirms first, then points you at **Copy MCP Config to Clipboard**. (#65)
- **MCP server (experimental): workflow write tools** — when write tools are enabled, external MCP clients can edit, auto-layout, and run Rewst workflows (`buddy_workflow_edit`, `buddy_workflow_autolayout`, `buddy_workflow_run`). Every change requires a per-workflow approval inside VS Code, so an external client can never approve its own write. (#68)
- **Nightly pre-release channel** — every merge to `main` now publishes a (#66)
...
- Add the `buddy_search_organizations` read-only MCP tool for finding managed organizations by name substring.
- Add the `buddy_list_users` read-only MCP tool for listing users directly in one organization.
- Add the `buddy_list_roles` read-only MCP tool for listing roles defined in one organization.
- **MCP server (experimental): org variable write tools** — when write tools are enabled, external MCP clients can create, update, and delete Rewst configuration variables (`buddy_create_org_variable`, `buddy_update_org_variable`, `buddy_delete_org_variable`), each org-scoped and gated by a per-change approval inside VS Code. (#80)
...
- Added read-only MCP tools to search templates by name and list App Platform pages and sites in a Rewst org, plus browse the global Jinja filter catalog.
...
- Added the `buddy_resolve_reference` MCP tool for name-to-id resolution of Rewst objects.
- **MCP server (experimental): tag write tools** — when write tools are enabled, external MCP clients can create, update, and delete Rewst tags (`buddy_create_tag`, `buddy_update_tag`, `buddy_delete_tag`), each org-scoped and gated by a per-change approval inside VS Code. (#80)
- **MCP server (experimental): template write tools** — when write tools are enabled, external MCP clients can create, update, rename, and delete Rewst templates (`buddy_create_template`, `buddy_update_template_body`, `buddy_rename_template`, `buddy_delete_template`), each org-scoped and gated by a per-change approval inside VS Code. (#80)
- **Safely set trigger org activation** — the new `buddy_set_trigger_activation` MCP tool sets which orgs a trigger is activated for (and toggles auto-activate) without disturbing clone overrides, creating a reversible patch and reporting a before/after diff. (#186)
- Added read-only MCP tools to list a Rewst org's triggers, forms, tags, and trigger activation instances, plus a batch trigger health check (`buddy_get_trigger_error_status`).
- **Read a trigger in full** — the new `buddy_get_trigger` MCP tool surfaces a trigger's tags, activation orgs, clone overrides, criteria, and parameters, which the trigger list leaves out. (#184)
- **Safely edit trigger tags** — the new `buddy_set_trigger_tags` MCP tool adds, removes, or replaces a trigger's tags without dropping the others, creating a revertable patch and reporting a before/after diff each time. (#184)
- **MCP server (experimental): trigger enable/disable tool** — when write tools are enabled, external MCP clients can now turn a Rewst workflow trigger on or off with `buddy_set_trigger_enabled`. It runs against a single organization, re-verifies the trigger belongs to that org before changing it, and requires a per-change approval inside VS Code. (#80)
- **MCP server (experimental): workflow create/delete tools** — when write tools are enabled, external MCP clients can create an empty workflow (`buddy_create_workflow`) and permanently delete one (`buddy_delete_workflow`), each org-scoped, re-verified against the target org, and gated by a per-change approval inside VS Code. (#80)
- Add the `buddy_list_workflow_executions` read-only MCP tool for recent workflow execution summaries.
- Add the `buddy_latest_workflow_execution` read-only MCP tool for the latest execution of one workflow.
- Add the `buddy_get_workflow_execution_stats` read-only MCP tool for aggregate workflow execution status counts.
...
- Add the `buddy_list_workflow_tasks` read-only MCP tool for listing workflow tasks.
- Add the `buddy_list_workflow_patches` read-only MCP tool for workflow patch metadata history.
- Add the `buddy_get_workflow_patch` read-only MCP tool for fetching one workflow patch with its JSON Patch payload.
...
**Sub-workflow outputs:** `buddy_workflow_edit` gains a `set_output` operation for defining the values a workflow returns to its caller, and `buddy_workflow_get` now shows those outputs — so AI tools can build composed workflows instead of one giant canvas. (#120)

### Changed

- **MCP server (internal): chat tools converge onto the capability registry** — the Cage-Free Rewsty chat tools are now defined once in the shared capability registry instead of separate spec arrays. No change to the tools you see or how the assistant behaves. (#61)
...
- **Expanded test coverage** — Broad unit coverage added across utilities, UI, MCP, and sessions.
- **Workflow lint now suggests a self-defaulting Jinja counter for manual retry loops.** The `task-retry-configured` finding shows the `CTX.retry|d|int` idiom so you don't need an extra task to initialize the counter before the loop starts. (#184)
...
- **Workflow and Buddy tool reliability** — Workflow, GraphQL, and MCP tool routing now share the same definitions, reducing stale guidance and keeping chat and external-client behavior aligned.
...
- **Sub-org template links keep their real org** — Syncing a template that lives in a sub-organization no longer relabels its link with your main org, so link tools, "Open in Rewst", and sync now point at the correct sub-org. Existing mislabeled links self-correct on their next sync. (#96)
...
- **Workflow tooling reliability** — Advanced task fields, mocking, workflow search, run timeouts, and Jinja diagnostics now surface clearly instead of being silently lost or misleading. (#126)
- **Workflow tooling fixes** - workflow edits now prompt consistently, mock data must use the `mock_result` wrapper, workflow diagnose shows the executed path, and managed sub-org sessions resolve correctly. (#150)
...
- **Delete operations, and raw GraphQL mutations, now always prompt for approval, even after another change to the same resource was approved.** Approving a rename or update could previously pre-approve a later delete, or a raw mutation reusing the same scope, with no further prompt. (#177)
...
- **Template sync avoids false conflicts** — Metadata-only timestamp drift no longer blocks safe local uploads when the remote template body is unchanged.
...
**Task logs across accounts:** `buddy_execution_logs` no longer reports an empty result for executions owned by another signed-in session — it now checks every active session and accepts an optional `orgId` to target the right account directly. (#120)
...
- **Production hardening** — Hardened filename sanitization, cookie parsing, WebSocket URL construction, Jinja/template pattern matching, markdown fence parsing, and MCP org scoping.
