import { type GraphqlToolDeps } from '../tools/graphqlTool';
...
function requireDeps(deps: GraphqlToolDeps | undefined): GraphqlToolDeps {
	if (!deps) {
		throw new Error(
			'No active Rewst session for the workflow tools. Sign in through the MCP host session settings and retry.',
		);
	}
	return deps;
}
